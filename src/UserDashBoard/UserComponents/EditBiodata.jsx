  ...
  useEffect(() => {
    if (user?.email) {
      fetch(
        ...
        .then(data => {
          if (data && data._id) {
            setHasBiodata(true); // biodata ache
            reset(data); // prefill the form
          } else {
            setHasBiodata(false); // biodata nei
          }
          setLoading(false);
        })
        .catch(err => {
          toast.error('Failed to load biodata');
          setHasBiodata(false); // error holeo dhore nebo biodata nai
          setLoading(false);
        });
    }
  }, [user?.email, reset]);
  ...
  return (
    <div>
        ...
        {!hasBiodata && (
          <div className="text-center my-6">
            <p className="text-lg mb-2 text-gray-700">
              আপনি এখনো কোনো বায়োডাটা যুক্ত করেননি। অনুগ্রহ করে নিচের বাটনে
              ক্লিক করে যুক্ত করুন।
            </p>
            <NavLink
              to="/added-member"
              className="btn bg-pink-400 text-white p-2 rounded-2xl mx-auto flex justify-center w-fit"
            >
              Add Your Biodata
            </NavLink>
          </div>
      ...
      {/* <div className=" ">
        <NavLink
          to="/added-member"
          className="btn bg-pink-400 text-white p-2 rounded-2xl max-w-3xl mx-auto flex justify-center m-2"
        >
          Add Your Biodata
        </NavLink>
      </div> */}
